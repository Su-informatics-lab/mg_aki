...
# ═══════════════════════════════════════════════════════════════════
# INTERNAL: NON-AKI ENDPOINTS × eGFR (for group circulation)
# Single figure: ICD vs LLM color-coded where both sources exist
# Mortality excluded (already in main text Figure 3f)
# ═══════════════════════════════════════════════════════════════════
_INTERNAL_ENDPOINTS = [
    ("Delirium", [("encephalopathy_icd", "ICD"), ("encephalopathy_delirium", "LLM")]),
    ("POAF", [("poaf_icd", "ICD"), ("poaf", "LLM")]),
    ("Vent. arrhythmia", [("vent_arrhythmia", "ICD")]),
    ("Transfusion", [("transfusion", "LLM")]),
    ("Reintubation", [("reintubation", "LLM")]),
]
_SRC_STYLE = {
    "ICD": {"color": "#666666", "marker": "s", "fillstyle": "full", "label": "ICD"},
    "LLM": {
        "color": GREEN,
        "marker": "o",
        "fillstyle": "full",
        "label": "Note-extracted (MIMIC); =ICD (eICU)",
    },
}
...
def fig_internal_endpoints():
    ...
    fig.suptitle(
        "Non-AKI Endpoints \u00d7 eGFR (ICD vs Note-extracted)",
        fontsize=9,
        fontweight="bold",
        y=1.02,
    )
    plt.tight_layout()
    save(fig, "internal_endpoints")
...
ALL_FIGS = {
    ...
    "internal_endpoints": fig_internal_endpoints,
}
