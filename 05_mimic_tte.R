...
#
#   TTE all:    all patients (primary)
#   TTE hypoMg: Mg < 2.0 subgroup
# ─────────────────────────────────────────────────────────────────────
...
ps_vars <- c(
  ...
  "has_vasopressor", "first_mg_value"
)
...
for (v in c("first_k_value", "first_ca_value", "first_hr", "bmi", "baseline_egfr")) {
...
if ("delta_mg" %in% names(dat_clean)) {
  ...
  if (nrow(d_mg) > 20) tryCatch({
    ...
    results_list[["ALL_mg_elevation"]] <- tibble(
      model="ALL_mg_elevation", term="trt", estimate=s$estimate,
...
store("ALL_vent_arrhythmia", r_vt)
if (!is.null(r_vt))
...
ptbl(aki_r, "AKI: IPTW")
for (r in aki_r) store(paste0("ALL_aki_", r$label), r)
...
for (r in tw_r) store(paste0("ALL_tw_", r$label), r)
...
if ("peak_cr_ratio" %in% names(dat_clean)) {
  ...
  tryCatch({
    ...
    cat(sprintf("\n  Peak Cr ratio diff = %.3f (%.3f to %.3f), p=%.4f\n",
                s$estimate, s$conf.low, s$conf.high, s$p.value))
    results_list[["ALL_peak_cr"]] <- tibble(
      model="ALL_peak_cr", term="trt", estimate=s$estimate,
      conf.low=s$conf.low, conf.high=s$conf.high, p.value=s$p.value,
  ...
  for (ov in names(aki_oc)) {
    if (ov %in% names(dat_matched)) tryCatch({
      ...
      if (nrow(s) > 0) {
        ...
        s$label <- aki_oc[[ov]]; s$n_total <- nrow(dat_matched)
        results_list[[paste0("ALL_matched_", aki_oc[[ov]])]] <- s
      }
    }, error = function(e) NULL)
  }
}
...
store("ALL_sec_Hospital mortality", r_mort)
ptbl(list(r_mort), "Mortality")

# Negative controls
nc_r <- list(iptw_or(dat_clean, "nc_fracture", "Fracture"),
             iptw_or(dat_clean, "nc_uti", "UTI"))
ptbl(nc_r, "Negative controls")
for (r in nc_r) store(paste0("ALL_nc_", r$label), r)

# Neuro
neuro_r <- list(iptw_or(dat_clean, "neuro_delirium", "Delirium"),
                iptw_or(dat_clean, "neuro_encephalopathy", "Encephalopathy"))
ptbl(neuro_r, "Neuro (exploratory)")
for (r in neuro_r) store(paste0("ALL_neuro_", r$label), r)
